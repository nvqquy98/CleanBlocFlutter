...
class AppBloc extends BaseBloc {
  ...
  /// Output
  late Stream<void> onClearAllUserInfoSuccess;
  late Stream<bool> isDarkModeStream;

  AppBloc(this._clearAllUserInfoUseCase, this._getHasLoginUseCase, this._isDarkModeUseCase,
      this._saveIsDarkModeUseCase) {
    final _clearAllUserInfoController = PublishSubject<Unit>()..disposeBy(disposeBag);
    onClearAllUserInfo = () => _clearAllUserInfoController.add(Unit());

    final _themeController = PublishSubject<bool>();
    changeTheme = (isDarkMode) {
      _saveIsDarkModeUseCase(isDarkMode).then((_) => _themeController.add(isDarkMode));
    };

    /// prevent spam
    isDarkModeStream = _themeController.stream.throttleTime(const Duration(milliseconds: 500));
